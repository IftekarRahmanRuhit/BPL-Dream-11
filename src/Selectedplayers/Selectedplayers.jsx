...
const Selectedplayers = ({ selectedPlayers ,handleRemovePlayer ,handleAddMorePlayers }) => {
  ...
  return (
    ...
    <div className='flex flex-col gap-1'>
      {selectedPlayers.map((player) => (
        <div className=" flex justify-between items-center  gap-4 border-2 p-5 item rounded-xl mt-7">
          ...
          <div>
            <button onClick={() => handleRemovePlayer(player.id)} className='text-xl text-red-500'><RiDeleteBin5Fill /></button>
          </div>
    ...
    <div>
      <button onClick={handleAddMorePlayers} className='btn  mt-7 p-4 bg-[#E7FE29]'>Add More Players</button>
    </div>
    </>
